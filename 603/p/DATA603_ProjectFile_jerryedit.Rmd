...
best.subset<-regsubsets(Site_Energy~NumberofBuildings+Property_GFA+Emissions_CO2+ Natural_Gas, data=dataset, nv=4 ) 
summary(best.subset)
...
```{r}
higherordermodel1 <-lm(Site_Energy~NumberofBuildings+Emissions_CO2+I(Emissions_CO2^2)+Natural_Gas+Property_GFA+Emissions_CO2*Natural_Gas+Emissions_CO2*Property_GFA+Natural_Gas*Property_GFA, data=dataset)
...
```
```{r}
higherordermodel3 <-lm(Site_Energy~NumberofBuildings+Emissions_CO2+I(Emissions_CO2^2)+I(Emissions_CO2^3)+I(Emissions_CO2^4)+Natural_Gas+Property_GFA+Emissions_CO2*Natural_Gas+Emissions_CO2*Property_GFA+Natural_Gas*Property_GFA, data=dataset)
...
### Higher Order Term Partial F-tests:
...
$$
\begin{aligned}
H_0&:\beta_{p-q+1}=\beta_{p-q+2}=...=\beta_p=0 \mbox{(Higher Order Term are not significant) }\\
H_a&:\mbox{at least one }\beta_i \neq0 \mbox{(At least one Higher Order Term is significant) }
\end{aligned}
$$
...
```{r}
anova(interacmodel3,interacmodel2)
```
It gives a p-value of $0.0001895<0.05$, indicating that we should clearly reject the null hypothesis, which confirms that we dohave enough evidence add higher order terms in the model. Hence, we can conclude that the model below is the best fitted model:
$$
\begin{aligned}
...
The results show that the model with the shortest Cp and AIC values and the highest R2adj values, demonstrating that the independent variables selected by the Stepwise Regression Procedure are the best candidates for modelling.
